...
## Define Ridge Model
```{r}
ridge_model <- logistic_reg(mode = "classification", 
                         engine = "glmnet", 
                         penalty = 0.01873817, 
                         mixture = 0)
```
...
## Knn: Predict on the Test Set
```{r}
knn_results <- bookings_test_solutions %>% select(is_cancelled) %>% 
  bind_cols(knn_wf_fit %>% 
              predict(new_data = bookings_test_solutions)) %>% 
  bind_cols(knn_wf_fit %>% 
              predict(new_data = bookings_test_solutions, type = "prob"))

```
## Ridge: Predict on the Test Set
```{r}
ridge_results <- bookings_test_solutions %>% select(is_cancelled) %>% 
  bind_cols(ridge_wf_fit %>% 
              predict(new_data = bookings_test_solutions)) %>% 
  bind_cols(ridge_wf_fit %>% 
              predict(new_data = bookings_test_solutions, type = "prob"))

```
## Lasso: Predict on the Test Set
```{r}
lasso_results <- bookings_test_solutions %>% select(is_cancelled) %>% 
  bind_cols(lasso_wf_fit %>% 
              predict(new_data = bookings_test_solutions)) %>% 
  bind_cols(lasso_wf_fit %>% 
              predict(new_data = bookings_test_solutions, type = "prob"))
...
```{r}
eval_metrics(data = knn_results, truth = is_cancelled, estimate = .pred_class)
```

# Ridge Model: Evaluation Metrics
```{r}
eval_metrics(data = ridge_results, truth = is_cancelled, estimate = .pred_class)
```

# Lasso Model: Evaluation Metrics
```{r}
eval_metrics(data = lasso_results, truth = is_cancelled, estimate = .pred_class)
```
...
params <- list(
  ...
  , feature_fraction = 22/ncol(bookings_train_pred)
)

model <- lgb.train(
  params = params
  , data = dtrain
  , num_iterations = 3600L
)
...
knn_roc <- roc_curve(knn_results, .pred_yes, truth = is_cancelled)
ridge_roc <- roc_curve(ridge_results, .pred_yes, truth = is_cancelled)
lasso_roc <- roc_curve(lasso_results, .pred_yes, truth = is_cancelled)

colors <- c("LightGBM"="blue", "XGBoost"="darkred", "Random Forest"="darkgreen",
            "Log. Reg."="darkorange", "knn"='darksalmon', 'ridge'='gold', 'lasso'='deeppink')
...
ggplot(lgbm_roc, aes(x = 1 - specificity, y = sensitivity, color='LightGBM')) +
  ...
  geom_path(data = knn_roc, aes(x = 1 - specificity, y = sensitivity, color='knn'), alpha=.5, size=1) +
  geom_path(data = ridge_roc, aes(x = 1 - specificity, y = sensitivity, color='ridge'), alpha=.5, size=1) +
  geom_path(data = lasso_roc, aes(x = 1 - specificity, y = sensitivity, color='lasso'), alpha=.5, size=1) +
  geom_abline(lty = 3) +
...
knn_pr <- pr_curve(knn_results, .pred_yes, truth = is_cancelled)
ridge_pr <- pr_curve(ridge_results, .pred_yes, truth = is_cancelled)
lasso_pr <- pr_curve(lasso_results, .pred_yes, truth = is_cancelled)
...
ggplot(lgbm_pr, aes(x = recall, y = precision, color='LightGBM')) +
  ...
  geom_path(data = knn_pr, aes(x = recall, y = precision, color='knn'), alpha=.5, size=1) +
  geom_path(data = ridge_pr, aes(x = recall, y = precision, color='ridge'), alpha=.5, size=1) +
  geom_path(data = lasso_pr, aes(x = recall, y = precision, color='lasso'), alpha=.5, size=1)
...
knn_auc <- roc_auc(knn_results, .pred_yes, truth = is_cancelled)
ridge_auc <- roc_auc(ridge_results, .pred_yes, truth = is_cancelled)
lasso_auc <- roc_auc(lasso_results, .pred_yes, truth = is_cancelled)
#knn
print(lgbm_auc)
print(rf_auc)
print(xgb_auc)
