...
```{r}
set.seed(82001)
cv_folds <- bookings_train_train |> vfold_cv(v = 4, strata = is_cancelled)
```
...
```{r}
recipe_prelim <- recipe(is_cancelled ~ ., data = bookings_train_train) |> 
  update_role( "date_col", "arrival_date_month", new_role = "metadata") |>
...
```{r}
set.seed(7)
lgbm_grid <- grid_max_entropy(learn_rate(range = c(-2.25, -1.85)),
                              trees(range = c(1500, 2500)),
                              min_n(range = c(3, 10)),
                              tree_depth(range=c(8, 11)),
                              mtry(range=c(6, 20)),
                              size = 75)
lgbm_grid
```
